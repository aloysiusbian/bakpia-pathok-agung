    ...
    <div class="container py-4">
          ...
          @else
            ...
            <div class="border rounded-3 p-3 bg-white mb-3 d-flex justify-content-between align-items-center">
              ...
              <form action="{{ route('keranjang.clear') }}" method="POST"
                ...
                @csrf
                <button type="submit" class="btn btn-link text-danger text-decoration-none fw-bold p-0 small">Hapus
                  Semua</button>
              </form>
            </div>
                ...
                <div class="border rounded-3 p-3 bg-white shadow-sm">
                  <div class="row align-items-center">
                    ...
                    <div class="col-3 col-md-2 d-flex align-items-center gap-2">
                      <input class="form-check-input item-checkbox" type="checkbox" data-id="{{ $item->idKeranjang }}"
                        checked>

                      <img src="{{ asset('images/' . $item->gambar) }}" alt="{{ $item->produk->namaProduk ?? 'Produk' }}"
                        class="rounded border" style="width: 100%; aspect-ratio: 1/1; object-fit: cover;"
        ...
        <div class="col-lg-4">
          <form id="checkout-form" action="{{ route('pembayaran.checkout') }}" method="POST">
            ...
            <div class="border rounded-3 p-3 bg-white position-sticky" style="top: 2rem;">
              ...
              <button type="submit" class="btn w-100 text-white fw-bold" style="background-color:#000; border-color:#000;"
      ...
      <div class="d-flex flex-row overflow-auto gap-3 pb-2">
        @forelse ($produksLainnya as $product)
          <a href="{{ route('produk.show', $product->idProduk) }}" class="text-decoration-none text-dark flex-shrink-0"
            style="width: 220px;">
            <div class="product-card shadow-sm rounded-4 border overflow-hidden bg-white" style="transition:.2s;">
              <img src="{{ asset('images/' . $product->gambar) }}" alt="{{ $product->namaProduk }}" class="w-100"
              ...
              <div class="product-info p-3">
                ...
                <form action="{{ route('keranjang.store') }}" method="POST" class="mt-3">
                  ...
                  <button type="submit" class="btn btn-add-cart w-100 fw-semibold" {{ $product->stok < 1 ? 'disabled' : '' }}>
                    + Keranjang
                  </button>
                </form>
              </div>
